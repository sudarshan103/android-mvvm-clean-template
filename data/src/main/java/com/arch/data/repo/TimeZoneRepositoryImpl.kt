...
class TimeZoneRepositoryImpl
    ...
    constructor(
        ...
        override fun getTimeZone(
            timezone: String,
            showProgress: Boolean
        ): Flow<Result<TimeZone>> =
            ApiCallHandler
                .executeApiCall(showProgress = showProgress) {
                    apiService.getTimeZone(timezone)
                }.map { response ->
                    mapToResult(response)
                }

        override fun getUtcTime(showProgress: Boolean): Flow<Result<TimeZone>> = getTimeZone(UTC_TIMEZONE, showProgress)
